...
def safe_rmtree(path, retries=5, delay=1.0):
    ...
    for attempt in range(retries):
        ...
        # Hint to Python to release any lingering file handles, then wait
        try:
            gc.collect()
        except Exception:
            pass
    ...
    # Final best-effort manual cleanup (don't raise)
    try:
        ...
        try:
            ...
            # couldn't delete everything; leave and return False
            return False
    except Exception:
        return False
...
wpa_supplicant = ""
jpgs = []
his_files = []  # IMMO history files containing VINs
...
def cli_emit_progress(value):
    """Emit a simple progress token to stdout when running in CLI mode.

    External UIs (Electron main or Flask SSE consumer) can parse lines that
    begin with 'PROGRESS:' and update their progress bars accordingly.
    """
    try:
        if cli_mode:
...
def copy_jpegs(jpgs_list, report_dir, progress_bar, base_directory):
    ...
    for root, dirs, files in os.walk(base_directory):
        ...
        if "VehicleImmo" in root:   # IMMO test records
            for file in files:
                if file.endswith('.his'):
                    his_files.append(os.path.join(root, file))
...
def is_invalid_test_vin(vin):
    """Check if VIN is a test/placeholder/invalid VIN that should be filtered out"""
    # Check for repeating single character (e.g., AAAAAAAAAAAAAAAAA, 11111111111111111)
    if len(set(vin)) == 1:
        return True
    ...
    if vin in test_patterns or 'TEST' in vin.upper() or 'UNKNOWN' in vin.upper():
        return True
    
    # Check for VINs with too few unique characters (likely test data)
    # Real VINs should have diverse characters
    if len(set(vin)) < 5:
        return True
    
    # Check for obvious demo/test patterns like LVSAAAAAAAA999999
    if vin.count('A') > 10 or vin.count('9') > 10 or vin.count('0') > 10:
        return True
        ...
        try:
            ...
            with open(log_file, 'r', encoding=encoding, errors='ignore') as file:
                ...
                for vin in found_vins:
                    try:
                        # Skip if all digits (likely a serial number, not a VIN)
                        if vin.isdigit():
                            continue
                        
                        # Skip test/placeholder VINs
                        if is_invalid_test_vin(vin):
                            print(f"Skipping test/invalid VIN: {vin}")
                            continue
                        
                        # Try to validate with vininfo
                        vin_obj = Vin(vin)
                        checksum_valid = False
                        
                        try:
                            checksum_valid = vin_obj.verify_checksum()
                        except Exception:
                            # Checksum validation failed - this is OK for international VINs
                            pass
                        
                        # Accept the VIN if:
                        # 1. Checksum is valid (US VINs), OR
                        # 2. vininfo can identify manufacturer/country (international VINs)
                        if checksum_valid or (vin_obj.manufacturer and vin_obj.country):
                            vins.add(vin)
                            hit_list.append(f"VIN: {vin}, File: {log_file}")
                            if not checksum_valid:
                                print(f"International VIN extracted (no US check digit): {vin} - {vin_obj.manufacturer} ({vin_obj.country})")
                        
                    except Exception as e:
                        # If vininfo fails completely, still extract if it looks valid
                        # (17 chars, no invalid characters, not all digits)
                        if len(vin) == 17 and not vin.isdigit() and not is_invalid_test_vin(vin):
                            vins.add(vin)
                            hit_list.append(f"VIN: {vin}, File: {log_file}")
                            print(f"VIN extracted (could not validate): {vin}")
        except Exception as e:
            print(f"Error reading {log_file}: {e}")
        
    return vins

def extract_vins_from_his_files(his_files, progress_bar):
    """Extract VINs from .his (history) files in VehicleImmo folders"""
    print(f"Extracting VINs from {len(his_files)} .his files...")
    vins = set()
    vin_pattern = re.compile(r'\b([A-HJ-NPR-Z0-9]{17})\b')
    
    for his_file in his_files:
        try:
            # .his files are simple text files, one entry per line
            with open(his_file, 'r', encoding='utf-8', errors='ignore') as file:
                content = file.read()
                found_vins = vin_pattern.findall(content)
                
                for vin in found_vins:
                    # Skip if all digits (likely something else)
                    if vin.isdigit():
                        continue
                    
                    # Skip test/placeholder VINs
                    if is_invalid_test_vin(vin):
                        print(f"Skipping test/invalid VIN from .his file: {vin}")
                        continue
                    ...
                    try:
                        ...
                        if checksum_valid or (vin_obj.manufacturer and vin_obj.country):
                            vins.add(vin)
                            hit_list.append(f"VIN: {vin}, File: {his_file}")
                            print(f"VIN extracted from IMMO history: {vin} ({os.path.dirname(his_file)})")
                    except Exception:
                        # Still add if it looks like a valid VIN format
                        if len(vin) == 17 and not vin.isdigit() and not is_invalid_test_vin(vin):
                            vins.add(vin)
                            hit_list.append(f"VIN: {vin}, File: {his_file}")
                            print(f"VIN extracted from IMMO history (unvalidated): {vin}")
                
        except Exception as e:
            print(f"Error reading {his_file}: {e}")
    
    print(f"Found {len(vins)} VINs in IMMO history files")
    return vins
...
def lookup_and_report_vins(vins, json_files):
    ...
    for vin in vins:
        ...
        # Try NHTSA API lookup first (US vehicles)
        try:
            vehicle = pyvin.VIN(vin)
            
            # Check if we got useful data from NHTSA
            if vehicle.Make and vehicle.Make.strip() and vehicle.Make != "Not Available":
                vin_entry.update({
                    "Make": vehicle.Make,
                    "Model": vehicle.Model if vehicle.Model else "N/A",
                    "Trim": vehicle.Trim if vehicle.Trim else "N/A",
                    "Year": vehicle.ModelYear if vehicle.ModelYear else "N/A",
                    "Type": vehicle.VehicleType if vehicle.VehicleType else "N/A",
                    "Source": "NHTSA API"
                })
                data_source = "NHTSA API"
            else:
                # NHTSA returned no useful data, use offline decoder
                raise ValueError("No useful NHTSA data")
                
        except Exception as e:
            # If PyVIN fails or returns no data, use vininfo (offline international decoder)
            try:
                vin_info = Vin(vin)
                
                # Extract data from vininfo
                country = vin_info.country or "Unknown"
                manufacturer = vin_info.manufacturer or "Unknown"
                region = vin_info.region or "Unknown"
                years = vin_info.years or "Unknown"
                ...
                # Create manufacturer string with country info
                make_str = manufacturer
                if country and country != "Unknown":
                    make_str = f"{manufacturer} ({country})"
                ...
                vin_entry.update({
                    ...
                    "Trim": "N/A", 
                    "Year": year_str,
                    "Type": region if region and region != "Unknown" else "N/A",
                    "Source": "Offline Decode"
                })
                data_source = "Offline Decode"
                
            except Exception as vininfo_error:
                # Both methods failed - VIN may be invalid
                vin_entry.update({
                    "Make": "Invalid VIN",
                    "Model": "N/A",
                    "Trim": "N/A",
                ...
                print(f"Unable to decode VIN {vin}: {vininfo_error}")
        
        vin_data.append(vin_entry)
    
    # Convert vin_data list into a DataFrame and return it
    df = pd.DataFrame(vin_data)
    print(f"VIN Lookups complete: {len([v for v in vin_data if v.get('Source') == 'NHTSA API'])} from NHTSA, "
          f"{len([v for v in vin_data if v.get('Source') == 'Offline Decode'])} from offline decode")
    return df
...
def generate_html_report(report_file, nickname, email, phone, address, city, state, country, 
    ...
    try:
        ...
        report_dir = os.path.dirname(report_file) if report_file else None
        if report_dir:
            try:
                if not os.path.exists(report_dir):
                    os.makedirs(report_dir, exist_ok=True)
                    print(f"DEBUG: Created report directory: {report_dir}")
                else:
                    print(f"DEBUG: Report directory exists: {report_dir}")
            except Exception as e:
                print(f"DEBUG: Failed to ensure report directory {report_dir}: {e}")

        print(f"DEBUG: Writing report to {report_file}")
        with open(report_file, 'w', encoding='utf-8') as f:
            f.write(html_content)
        print(f"DEBUG: Report successfully written to {report_file}")
    except Exception as e:
        print(f"ERROR: Failed to write report to {report_file}: {e}")
        raise
...
def run_parser(input_path, report_file, progress_bar):
    ...
    if zipfile.is_zipfile(input_path):
        ...
        try:
            ...
            # attempt to remove temp dir but be tolerant of permission errors on Windows
            try:
                safe_rmtree(temp_dir)
            except Exception:
                print(f"DEBUG: Failed to fully remove temp dir {temp_dir} after extraction error; leaving it in place.")
            return
    else:
        base_directory = input_path

    log_files, json_files = find_log_files(base_directory)
    # Phase: found log files
    cli_emit_progress(10)
    total_steps = len(log_files)
    # If the caller passed the CLI DummyProgress, enable CLI progress emissions
    try:
        global cli_mode
        if hasattr(progress_bar, '__class__') and progress_bar.__class__.__name__ == 'DummyProgress':
            cli_mode = True
    except Exception:
        pass
    ...
    # Phase: parsed xTool/profile/shared prefs
    cli_emit_progress(40)
    ...
    # Extract VINs from IMMO history files
    his_vins = extract_vins_from_his_files(his_files, progress_bar)
    vins = vins.union(his_vins)
    
    cli_emit_progress(80)
    vin_lookups = lookup_and_report_vins(vins, json_files)
    cli_emit_progress(90)
    ...
    # Signal completion to any UI parsing stdout
    progress_bar.set(100)
    cli_emit_progress(100)
    if zipfile.is_zipfile(input_path):
        try:
            removed = safe_rmtree(temp_dir)
            if removed:
                print(f"DEBUG: Temp dir removed: {temp_dir}")
            else:
                print(f"DEBUG: Temp dir not fully removed (files may be locked): {temp_dir}")
        except Exception:
            print(f"DEBUG: Failed to remove temp dir {temp_dir} after successful run; it may be locked by another process.")
...
def show_help():
    ...
    Results:
    - VINs extracted from log files and JSON files. Vehicle details are obtained via NHTSA API (US vehicles) or offline VIN decoder (international vehicles).
...
def _run_cli_mode():
    ...
    class DummyProgress:
        def __init__(self):
            self.max = 0
        def set(self, v):
            # emit progress to stdout so external UIs can parse it
            try:
                cli_emit_progress(v)
            except Exception:
                pass
...
if __name__ == '__main__':
    # If --cli is passed, run CLI mode and exit. This preserves the existing GUI behavior
    # when no flag is passed while enabling the Electron/main process to call the script
    # with `--cli <input> <report>`.
    if '--cli' in sys.argv:
        _run_cli_mode()
        sys.exit(0)
