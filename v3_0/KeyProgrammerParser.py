...
#to do 
# html report template path and output
...
def parse_vehiclehistory_db(vehiclehistory_db, report_file, progress_bar):
    ...
    if not vehiclehistory_db:
        ...
        try:
            ...
            print(f"Error connecting to VehicleHistory.db: {e}")
        except Exception as e:
            print(f"Error connecting to VehicleHistory.db: {e}")
...
def generate_html_report(report_file, nickname, email, phone, address, city, state, country, 
                         ...
                         ViH_df, VeH_df, vin_lookups, sources, template_path):
...
def run_parser(input_path, report_file, progress_bar):
    ...
    additional_vins = ViH_vins + VeH_vins
...
def show_help():
    ...
    KeyProgrammerParser v3.0
    -------------------------
    This tool parses log files, JSON files, and VIN history databases to extract VIN and user data from Autel KM100x & IM508 devices.
...
root = ctk.CTk()
root.title("KeyProgrammerParser v3.0")
